...
void TransactionManager::registerTransaction(
    TransactionState& state, std::unique_ptr<TransactionData> data) {
  ...
  _transactions[bucket]._activeTransactions.emplace(state.id(),
                                                    std::move(data));
}
...
TransactionState* TransactionManager::lookup(
    TRI_voc_tid_t transactionId, TransactionManager::Ownership action) const {
  size_t bucket = getBucket(transactionId);
  READ_LOCKER(allTransactionsLocker, _allTransactionsLock);
